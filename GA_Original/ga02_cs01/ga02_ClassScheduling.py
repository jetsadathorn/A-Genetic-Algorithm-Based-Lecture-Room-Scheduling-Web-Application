...
class Data:
    ...
    MEETING_TIMES = [["จ.เช้า", "จ 09:00 - 12:00"],
                     ["จ.บ่าย", "จ 13:00 - 16:00"],
                     ["อ.เช้า", "อ 09:00 - 12:00"],
                     ["อ.บ่าย", "อ 13:00 - 16:00"],
                     ["พ.เช้า", "พ 09:00 - 12:00"],
                     ["พ.บ่าย", "พ 13:00 - 16:00"],
                     ["พฤ.เช้า", "พฤ 09:00 - 12:00"],
                     ["พฤ.บ่าย", "พฤ 13:00 - 16:00"],
                     ["ศ.เช้า", "ศ 09:00 - 12:00"],
                     ["ศ.บ่าย", "ศ 13:00 - 16:00"]]
    INSTRUCTORS = [["I1", "ผศ.ดร.ธีระ"],
                   ["I2", "ผศ.ดร.วิสันต์"],
                   ["I3", "ผศ.ดร.ศรัณย์"],
                   ["I4", "อ.สันธนะ"]]
    ...
    def __init__(self):
        ...
        course1 = Course("05506001", "DISCRETE MATHEMATICS", [
                         self._instructors[0], self._instructors[1]], 25)
        course2 = Course("05506004", "OBJECT-ORIENTED PROGRAMMING",
                         [self._instructors[0], self._instructors[1], self._instructors[2]], 35)
        course3 = Course("05506008", "COMPUTER ORGANIZATION AND ARCHITECT", [
                         self._instructors[0], self._instructors[1]], 25)
        course4 = Course("05506015", "จรรยาบรรณทางวิชาชีพและเชิงสังคม", [
                         self._instructors[2], self._instructors[3]], 30)
        course5 = Course("05506233", "CALCULUS FOR COMPUTER SCIENCE", [
                         self._instructors[3]], 35)
        course6 = Course("90641002", "ความฉลาดทางดิจิทัล", [
                         self._instructors[0], self._instructors[2]], 45)
        course7 = Course("90644008", "FOUNDATION ENGLISH 2", [
                         self._instructors[1], self._instructors[3]], 45)
        self._courses = [course1, course2, course3,
                         course4, course5, course6, course7]
        dept1 = Department("ชีวะ", [course1, course3])
        dept2 = Department("ฟิสิกส์", [course2, course4, course5])
        dept3 = Department("วิทยาการคอมพิวเตอร์", [course6, course7])
...
class Schedule:
    ...
    def calculate_fitness(self):
        ...
        for i in range(len(classes)):
            ...
            key = classes[i].get_dept()
            if key not in groups:
                groups[key] = set()

            meeting_time = classes[i].get_meetingTime()
            if meeting_time not in groups[key]:
                groups[key].add(meeting_time)
            else:
                self._numbOfConflicts += 1

        self._fitness = 1 / (self._numbOfConflicts + 1)  # Update fitness value
        self._numbOfGroups = len(groups)  # Store number of unique groups
        return self._fitness
...
def check123(population):
    data2 = str(population.get_schedules()[0]).split(" ")
    result = []
    current_subject = ""
    current_group = []
    for item in data2:
    ...
    for x in result:
        ...
        for y in x:
            # print(y,":", y.split(",")[4])
            checker.append(y.split(",")[4])
            # print(checker, "->", len(checker) != len(set(checker)))
...
while (population.get_schedules()[0].get_fitness() != 1.0):
    ...
    population.get_schedules().sort(key=lambda x: x.get_fitness(), reverse=True)
    print(str(population.get_schedules()[0]).split(" "))
    print("check:", check123(population))
    # displayMgr.print_generation(population)
    # displayMgr.print_schedule_as_table(population.get_schedules()[0])
print("\n\n")
